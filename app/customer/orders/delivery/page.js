...
export default function DeliveryPage() {
  ...
  const { orders, loading, meta } = useShopOrders({
    status: 'pushed_to_carrier,shipping',
    search: search.trim(),
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ marginBottom: 18 }}>
        <div>
          <div className="page-title">Đơn giao hàng</div>
          <div className="page-subtitle">{meta.total || orders.length} đơn đã đẩy vận chuyển hoặc đang giao, cập nhật từ carrier mock.</div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="table-wrapper">
          <table className="table">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={10} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Đang tải đơn giao hàng...</td></tr>
              ) : orders.length === 0 ? (
                <tr><td colSpan={10} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Không có đơn nào đang giao.</td></tr>
              ) : orders.map((order) => (
                <tr key={order.id}>
                  <td><Link href={`/customer/orders/${order.id}`} style={{ fontFamily: 'monospace', fontWeight: 800, color: 'var(--primary)' }}>{order.code}</Link></td>
                  ...
                  <td style={{ textAlign: 'right', fontWeight: 800, color: '#047857' }}>{formatCurrency(order.codAmount)}</td>
