  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ marginBottom: 18 }}>
        <div>
          <div className="page-title">Ký nhận một phần</div>
          <div className="page-subtitle">
            {meta.total || orders.length} đơn giao một phần: khách chỉ nhận một phần hàng, COD cần theo dõi riêng khi đối soát.
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="table-wrapper">
          <table className="table">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Đang tải đơn giao một phần...</td></tr>
              ) : orders.length === 0 ? (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Không có đơn giao một phần phù hợp.</td></tr>
              ) : orders.map((order) => (
                <tr key={order.id}>
                  <td>
                  ...
                  <td style={{ textAlign: 'right', fontWeight: 800, color: '#b45309' }}>{formatCurrency(order.codAmount)}</td>
