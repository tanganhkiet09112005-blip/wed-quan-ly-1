...
async function requestJT(msgType, urlType, payloadObj, credentials) {
  ...
  // Production guard for endpoints
  if (mode === 'production' && targetUrl.includes('demo-ylstandard')) {
    throw new Error('Chế độ Production yêu cầu cấu hình Endpoint thật từ J&T (hiện đang dùng link demo mặc định). Vui lòng cấu hình URL trong trang Đơn vị vận chuyển.');
  }
...
jtAdapter.createShipment = async (order, credentials = {}) => {
  ...
  try {
    ...
    const payload = {
      ...
      goodsvalue: order.total || 0,
      weight: order.weight || 1,
      length: order.length || undefined,
      width: order.width || undefined,
      height: order.height || undefined,
      servicetype: '1',
      items: [{ itemName: 'Sản phẩm', itemValue: order.total || order.codAmount || 0, itemQuantity: 1 }],
      paytype: '1',
    };
