  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ marginBottom: 18 }}>
        <div>
          <div className="page-title">Kiện vấn đề / đơn lỗi</div>
          <div className="page-subtitle">{meta.total || orders.length} đơn giao thất bại, mất hàng, hư hỏng hoặc cần shop xử lý.</div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="table-wrapper">
          <table className="table">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={10} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Đang tải kiện vấn đề...</td></tr>
              ) : orders.length === 0 ? (
                <tr><td colSpan={10} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Không có kiện vấn đề phù hợp.</td></tr>
              ) : orders.map((order) => (
                <tr key={order.id}>
                  <td>
                    <Link href={`/customer/orders/${order.id}`} style={{ fontFamily: 'monospace', fontWeight: 800, color: '#b91c1c' }}>
                      {order.code}
                    </Link>
                    <div style={{ marginTop: 4 }}>
                      <span className={`badge ${getStatusColor(order.status)}`} style={{ gap: 4 }}>
                        <AlertTriangle size={12} /> {getStatusLabel(order.status)}
                      </span>
                    </div>
                  </td>
                  ...
                  <td style={{ fontSize: 12, color: '#b91c1c', maxWidth: 240 }}>{order.note || '-'}</td>
                  <td style={{ fontSize: 12, color: 'var(--text-muted)' }}>{formatDate(order.createdAt)}</td>
                  <td>
